        ...
        <nav className="max-w-[1600px] mx-auto px-8 lg:px-16">
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <span className="font-display italic text-[1.6rem] text-[#B15607] tracking-[0.14em]">Solara</span>
            </Link>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className="relative px-6 py-2.5 font-body text-base font-medium text-foreground/85 hover:text-foreground transition-colors"
                  >
                    {item.label}
                    {isActive && (
                      <motion.div
