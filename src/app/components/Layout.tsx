              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className="relative px-6 py-2.5 font-body text-base font-medium text-foreground/85 hover:text-foreground hover:font-bold transition-colors duration-200"
                  >
                    {item.label}
                    {isActive && (
                      <motion.div
                        layoutId="activeNav"
                        className="absolute inset-0 bg-primary/25 rounded-lg -z-10"
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-10 md:gap-16">
            <div className="col-span-2">
              <Link to="/" className="flex items-center gap-3 mb-4 w-fit">
                <span className="font-display italic text-[1.5rem] tracking-[0.12em] text-[#B15607]">Solara</span>
              </Link>
